...
> 一般来说，不建议把 `.eslintignore` 和 `.stylelintignore` 添加到 preset 中，因为 preset 应该对所有项目都是适用的（除非你的 preset 限定在一个很小的范围内使用，各个项目的目录结构都是一致的，此时可以使用它们来定义需要忽略校验的文件和文件夹）。
...
- npm：>= 3.8.6，同样建议使用新版本。另外部分版本有 bug，详情查看[常见问题](#53-安装完成后没有配置文件)。
