...
function getPreset(presetName) {
  ...
  Object.keys(presetModule).forEach(linterName => {
    if (typeof presetModule[linterName] === 'string'
      && path.isAbsolute(presetModule[linterName])) {
      preset[linterName] = presetModule[linterName];
    } else {
      preset[linterName] = path.join(presetPath, `${linterName}.json`);
    }
  });
