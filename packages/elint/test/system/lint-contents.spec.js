...
test('git add 符合规范的文件后，修改为不符合，commit 成功（验证格式）', async () => {
  // 添加符合规范的文件
  await run('git add src/standard1.css', tmpDir)
...
test('git add 符合规范的文件后，修改为不符合，commit 成功（多文件）（验证格式）', async () => {
  // 添加符合规范的文件
  await run('git add src/standard1.css', tmpDir)
  await run('git add src/standard2.css', tmpDir)
...
test('git add 不符合规范的文件后，修改为符合规范的，commit 不成功（验证格式）', async () => {
  // 添加不符合规范的文件
  await run('git add src/index.js', tmpDir)
...
test('git add 不符合规范的文件后，修改为符合规范的，commit 不成功（多文件）（验证格式）', async () => {
  // 添加不符合规范的文件
  await run('git add src/index.js', tmpDir)
  await run('git add src/standard.js', tmpDir)
